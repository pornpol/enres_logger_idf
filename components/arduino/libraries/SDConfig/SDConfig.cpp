...
bool SDConfig::begin(uint8_t cs, Stream &debug)
{
  ...
  if (!SD.begin(_cs))
  {
    ...
    _debug->println("Init SD Card OK");
...
bool SDConfig::phaseConfig(String config)
{
  ...
  cfgG.numFlow = root["global"]["flows"];
  ...
  for (uint8_t i = 0; i < cfgG.numFlow; i++)
  {
    cfgF[i].id = root["flow"][i]["id"];
    cfgF[i].xid = root["flow"][i]["xid"];
    cfgF[i].type = root["flow"][i]["type"];
    for (uint8_t j = 0; j < 9; j++)
    {
      cfgF[i].adjust[j] = root["flow"][i]["adjust"][j];
    }
  }
