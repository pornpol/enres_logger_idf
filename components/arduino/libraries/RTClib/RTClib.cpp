...
////////////////////////////////////////////////////////////////////////////////
// RTC_MCP7940 implementation

boolean RTC_MCP7940::begin(void) {
  Wire.begin();
  return true;
}

uint8_t RTC_MCP7940::isrunning(void) {
...
void RTC_MCP7940::adjust(const DateTime& dt) {
  Wire.beginTransmission(MCP7940_ADDRESS);
  Wire._I2C_WRITE((byte)0); // start at location 0
  Wire._I2C_WRITE(bin2bcd(dt.second()) | 0x80);
  Wire._I2C_WRITE(bin2bcd(dt.minute()));
  Wire._I2C_WRITE(bin2bcd(dt.hour()));
  Wire._I2C_WRITE(0x8); //Enable VBAT
  Wire._I2C_WRITE(bin2bcd(dt.day()));
  Wire._I2C_WRITE(bin2bcd(dt.month()));
  Wire._I2C_WRITE(bin2bcd(dt.year() - 2000));
  Wire.endTransmission();
...
Mcp7940SqwPinMode RTC_MCP7940::readSqwPinMode() {
  int mode;

  Wire.beginTransmission(MCP7940_ADDRESS);
  Wire._I2C_WRITE(MCP7940_CONTROL);
  Wire.endTransmission();
  
  Wire.requestFrom((uint8_t)MCP7940_ADDRESS, (uint8_t)1);
  mode = Wire._I2C_READ();

  mode &= 0x93;
  return static_cast<Mcp7940SqwPinMode>(mode);
}

void RTC_MCP7940::writeSqwPinMode(Mcp7940SqwPinMode mode) {
  Wire.beginTransmission(MCP7940_ADDRESS);
  Wire._I2C_WRITE(MCP7940_CONTROL);
  Wire._I2C_WRITE(mode);
  Wire.endTransmission();
...
void RTC_MCP7940::readnvram(uint8_t* buf, uint8_t size, uint8_t address) {
  int addrByte = MCP7940_NVRAM + address;
  Wire.beginTransmission(MCP7940_ADDRESS);
  Wire._I2C_WRITE(addrByte);
  Wire.endTransmission();
  
  Wire.requestFrom((uint8_t) MCP7940_ADDRESS, size);
  for (uint8_t pos = 0; pos < size; ++pos) {
    buf[pos] = Wire._I2C_READ();
  }
}

void RTC_MCP7940::writenvram(uint8_t address, uint8_t* buf, uint8_t size) {
  int addrByte = MCP7940_NVRAM + address;
  Wire.beginTransmission(MCP7940_ADDRESS);
  Wire._I2C_WRITE(addrByte);
  for (uint8_t pos = 0; pos < size; ++pos) {
...
uint8_t RTC_MCP7940::readnvram(uint8_t address) {
  uint8_t data;
  readnvram(&data, 1, address);
  return data;
}

void RTC_MCP7940::writenvram(uint8_t address, uint8_t data) {
  writenvram(address, &data, 1);
}
///////////////////////////////////////////////////////////////////////////////////////////
...
void RTC_DS3231::readnvram(uint8_t* buf, uint8_t size, uint8_t address) {
  ...
  Wire.requestFrom((uint8_t) DS3231_ADDRESS, size);
  for (uint8_t pos = 0; pos < size; ++pos) {
    buf[pos] = Wire._I2C_READ();
  }
}
