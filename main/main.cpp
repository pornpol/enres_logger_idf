...
void ReadFlashMeterToPost_Task()
{
  ...
  if (wifiMulti.run() == WL_CONNECTED)
  {
    ...
    if (rIndex < wIndex)
    {
      ...
      {
        if (numPost > MAXPOST)
          numPost = MAXPOST; // Set Max POST per loop to prevent read meter
        Serial.printf("Number(s) to POST : %d\r\n", numPost);
        flashMeterToPost(numPost);
      }
      else
      {
        if (numPost > sd.cfgG.batch)
          numPost = sd.cfgG.batch;
        Serial.printf("Number(s) to POST : %d\r\n", numPost);
...
bool flowToFlash()
{
  ...
  for (int i = 0; i < sd.cfgG.numFlow; i++)
  {
    // inst_flux ins_heat_flow tm1 tm2 signal_ratio reynolds q_str up_str down_str
    *(uint32_t *)&wBuff[index] = sd.cfgF[i].xid; //[0, 4,294,967,295] <<-------- edit
    index += 4;
    *(uint32_t *)&wBuff[index] = flow.fd[i].mdt; //[0, 4,294,967,295]
    index += 4;
    *(float *)&wBuff[index] = flow.fd[i].inst_flux;
    index += 4;
    *(float *)&wBuff[index] = flow.fd[i].ins_heat_flow;
    index += 4;

    //
    // *(float*)&wBuff[index] = sensor.getSensor(0) * sd.cfgS.adjust[0][0] + sd.cfgS.adjust[0][1];
    *(float *)&wBuff[index] = flow.fd[i].tm1;
    index += 4;
    // *(float*)&wBuff[index] = sensor.getSensor(1) * sd.cfgS.adjust[1][0] + sd.cfgS.adjust[1][1];
    *(float *)&wBuff[index] = flow.fd[i].tm2;
    index += 4;

    *(float *)&wBuff[index] = flow.fd[i].signal_ratio;
    index += 4;
    *(float *)&wBuff[index] = flow.fd[i].reynolds;
    index += 4;
    *(float *)&wBuff[index] = flow.fd[i].q_str;
    index += 4;
    *(float *)&wBuff[index] = flow.fd[i].up_str;
    index += 4;
    *(float *)&wBuff[index] = flow.fd[i].down_str;
    index += 4;
  ...
  if (flash.writeFlowData(wIndex, wBuff, RECSIZE * sd.cfgG.numFlow))
  {
    Serial.println("Flash Write OK");
    wIndex += sd.cfgG.numFlow;
    // Save to RTC
    setWIndex();
    Serial.printf("Next Write Index : %d\r\n", wIndex);
    UNDERLINE;
...
{
  // Add Loop for multiple read
  for (uint8_t i = 0; i < sd.cfgG.numFlow; i++)
  {
    if (!flow.readFlowData(i, sd.cfgF[i].id, sd.cfgF[i].type, mktime(&timeinfo), sd.cfgF[i].adjust))
    {
